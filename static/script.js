...
document.getElementById('createSessionBtn').onclick = async () => {
  ...
  if (!hours || hours < 0.01) {
    hours = 0.01; // ✅ enforce minimum 0.01 hours
  }
...
async function fetchContacts() {
  try {
    ...
    // State will be handled by countdown
  } catch (err) {
    showMessage(err.message, 'error');
  }
}
...
function startCountdown() {
  setInterval(() => {
    ...
    if (diff <= 0) {
      ...
      downloadBtn.style.backgroundColor = "green"; // ✅ active state
